...
function pass() {
    ...
    pass += /^(A[\d]{2}28[\d]{3})$/.test($('#leader_number').val()) ? message($('.p_leader_name .help-inline'), '') : message($('.p_leader_name .help-inline'), '姓名&學號 不能為空 且學號階為大A') ;
    
    $('.p_std_number').each(function() {
        pass += $('.name', this).val().trim() != '' && /^(A[\d]{2}28[\d]{3})$/.test($('.num', this).val()) ? message($('.help-inline', this), '') : message($('.help-inline', this), '姓名&學號 不能為空 且學號階為大A');
